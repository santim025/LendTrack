...
describe("getPaymentsNeeded", () => {
  it("returns 0 when reference is the same month as start", () => {
    const start = new Date(2025, 3, 15) // April 2025
    const ref = new Date(2025, 3, 20) // April 2025
    expect(getPaymentsNeeded(start, ref)).toBe(0)
  })

  it("returns 1 when reference is one month after start", () => {
    const start = new Date(2025, 3, 15) // April 2025
    const ref = new Date(2025, 4, 10) // May 2025
  ...
  it("returns correct count across year boundary", () => {
    ...
    const ref = new Date(2025, 1, 15) // February 2025
  ...
  it("returns correct count for many months", () => {
    ...
    const ref = new Date(2026, 5, 12) // June 2026
...
describe("getPaymentDate", () => {
  ...
  it("handles February in leap year", () => {
    const start = new Date(2023, 11, 15) // December 2023
  ...
  it("handles February in non-leap year", () => {
    const start = new Date(2024, 11, 15) // December 2024
  ...
  it("payment from end-of-month start date", () => {
    const start = new Date(2025, 3, 30) // April 30, 2025
    const date = getPaymentDate(start, 1)
    expect(date.getFullYear()).toBe(2025)
    expect(date.getMonth()).toBe(4) // May
    expect(date.getDate()).toBe(31)
